...
#include "..\..\Common\MT5APIPublicKey.h"
...
//| Gateway API version                                              |
//+------------------------------------------------------------------+
#define MTGatewayAPIVersion  1881
#define MTGatewayAPIDate     L"09 Jul 2018"
//+------------------------------------------------------------------+
   ...
   enum EnModes
     {
      ...
      GATEWAY_MODE_REMOTE     =8,                           // work as remote service
      //--- enumeration borders
      GATEWAY_MODE_NONE       =0,
...
public:
   ...
   virtual void      OnGatewaySymbolAdd(LPCWSTR /*symbol*/)    {  }
   virtual void      OnGatewaySymbolDelete(LPCWSTR /*symbol*/) {  }
...
//--- remove module name
   if((ptr=wcsrchr(path,L'\\'))!=NULL)
      *(ptr+1)=L'\0';
//--- add Gateway API DLL name
   if(sizeof(void*)==4)
      wcscpy_s(name,_countof(name),L"MT5APIGateway.dll");
   else
      wcscpy_s(name,_countof(name),L"MT5APIGateway64.dll");
//--- set path
   ...
   if((m_hmodule=::LoadLibrary(path))==NULL)
     {
      ...
      //--- set root path
      wcscat_s(path,_countof(path),name);
      //--- try to load Gateway API DLL from root folder
...
//--- check result
   if(m_hmodule && m_mtcreate && m_mtcreate_local)
      res=MT_RET_OK;
//--- show error message
   if(res==MT_RET_ERROR)
      wprintf(L"\n  File '%s' does not exist or corrupted\n",path);
//--- return result
   return(res);
  }
//+------------------------------------------------------------------+
